...
import {find, map, share} from 'rxjs/operators';
...
export class PortfolioService {
  ...
  public getResponses(currentPage: number, pageNumber: number):Observable<any> {
  ...
  public getResponsesByFilter(currentPage: number, pageNumber: number, searchData: Search, statusId: number, sortingOption: { fieldName: string, sortType: number }):Observable<any> {
  ...
  public getResponseById(id:number): Observable<Portfolio>{
    return this.getResponsesMapData().pipe(
      map(portfolios => {
        return portfolios?.find(portfolio => portfolio?.InterventionInstanceId === id);
      })
    )
  }
